...
class CustomDefaultdict(defaultdict):
    ...
    def sample(self, batch_size, argument):
        ...
        dict_min = {
            k: v[min_idx * batch_size : (min_idx + 1) * batch_size] if not "loss" in k else v[min_idx : min_idx + 1]
            for k, v in self.items()
        }
        dict_max = {
            k: v[max_idx * batch_size : (max_idx + 1) * batch_size] if not "loss" in k else v[max_idx : max_idx + 1]
            for k, v in self.items()
        }
...
def laod_llm2vec(args, pretrained_mode_dir):
    ...
    if pretrained_mode_dir:
        ...
        embedding_model = PeftModel.from_pretrained(embedding_model, pretrained_mode_dir)
    else:
