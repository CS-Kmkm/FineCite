...
class ExtractionModel(torch.nn.Module):
    ...
    def forward(self, **inputs):
        ...
        if (
            "llm2vec" in self.model_name
        ):  # the llm2vec model have a next token embedding, thus to align the model with the labels it has to be rolled forward one postion
        ...
        for _, data in enumerate(data_loader, 0):
            with torch.no_grad():
                ...
                mask = (
                    1
                    - (
                        (input_ids == self.args.pad_token_id).to(torch.int)
                        | (input_ids == self.args.cls_token_id).to(torch.int)
                        | (input_ids == self.args.sep_token_id).to(torch.int)
                    )
                ).to(self.device)
                ...
                if self.args.ext_type in ["crf", "bilstm_crf"]:
                    ...
                    preds = [
                        [p if m else -100 for p, m in zip(preds, mask)]
                        for preds, mask in zip(torch.argmax(token_labels, dim=1).tolist(), (mask == 1).tolist())
                    ]
