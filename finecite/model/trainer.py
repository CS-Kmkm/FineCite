...
class CustomTrainer:
    def __init__(self, args, model, tokenizer, train_dataloader, test_dataloader):
        self.args = args
        self.model = model
        ...
        self.metric_multi_acc = torchmetrics.Accuracy(task="multiclass", num_classes=self.args.num_labels).to(
            self.device
        )
        ...
        if self.args.dataset == "multicite":
            self.metric_macro_f1 = torchmetrics.F1Score(
                average="macro", num_labels=self.args.num_labels, task="multilabel"
            ).to(self.device)
            self.metric_micro_f1 = torchmetrics.F1Score(
                average="micro", num_labels=self.args.num_labels, task="multilabel"
            ).to(self.device)
            self.metric_precision = torchmetrics.Precision(
                average=None, task="multilabel", num_labels=self.args.num_labels
            ).to(self.device)
            self.metric_recall = torchmetrics.Recall(
                average=None, task="multilabel", num_labels=self.args.num_labels
            ).to(self.device)
            self.metric_f1 = torchmetrics.F1Score(average=None, task="multilabel", num_labels=self.args.num_labels).to(
                self.device
            )
        else:
            self.metric_macro_f1 = torchmetrics.F1Score(
                average="macro", num_classes=self.args.num_labels, task="multiclass"
            ).to(self.device)
            self.metric_micro_f1 = torchmetrics.F1Score(
                average="micro", num_classes=self.args.num_labels, task="multiclass"
            ).to(self.device)
            self.metric_precision = torchmetrics.Precision(
                average=None, task="multiclass", num_classes=self.args.num_labels
            ).to(self.device)
            self.metric_recall = torchmetrics.Recall(
                average=None, task="multiclass", num_classes=self.args.num_labels
            ).to(self.device)
            self.metric_f1 = torchmetrics.F1Score(average=None, task="multiclass", num_classes=self.args.num_labels).to(
                self.device
            )
    ...
    def _configure_optimizer(self):
        ...
        # CRF params live under `self.model.crf` (e.g. `crf.transitions`).
        # Keep them in a dedicated optimizer with its own learning rate.
        crf_param = "crf"
        ...
        if self.args.ext_type in ["crf", "bilstm_crf"]:
            crf_optimizer_grouped_parameters = [
                {
                    "params": [p for n, p in self.model.named_parameters() if p.requires_grad and crf_param in n],
                    "weight_decay": self.args.weight_decay,
                },
            ]
            ...
            for i, data in enumerate(eval_dataloader, 0):
                ...
                if self.args.task == "ext":
                    eval_data.extend(
                        {
                            "ext_out": output,
                            "tok_lbl": data["token_labels"][:, 0].to(self.device),
                            "tok_lbl2": data["token_labels"][:, 1].to(self.device),
                        }
                    )
    ...
    def create_output_samples(self, samples):
        ...
        if "cls" in self.args.task:
            ...
            for idx, (ids, preds, int_lbl) in enumerate(
                zip(samples["input_ids"], samples["cls_out"], samples["int_lbl"])
            ):
                ...
                if self.args.task == "multicite":
                    ...
                    output_samples += (
                        "Preds:  " + [idx for idx in range(self.args.num_labels) if preds[idx] >= 0.5] + "\n"
                    )
                    output_samples += (
                        "Labels1:" + [idx for idx in range(self.args.num_labels) if int_lbl[idx] >= 0.5] + "\n\n"
                    )
                else:
    ...
    def ext_sample_string(self, samples):
        sample_string = "\n"
        for idx, (ids, logits, lbl1, lbl2) in enumerate(
            zip(samples["input_ids"], samples["ext_out"], samples["tok_lbl"], samples["tok_lbl2"])
        ):
    ...
    def on_evaluation_epoch_end(self, eval_data):
        ...
        if "ext" in self.args.task and self.args.dataset == "finecite":
            ...
            labels1, labels2 = (
                eval_data["tok_lbl"][eval_data["tok_lbl"] != -100],
                eval_data["tok_lbl2"][eval_data["tok_lbl"] != -100],
            )
            labels1, labels2 = (
                torch.tensor([l if l <= 3 else l - 3 for l in labels1]).to(self.device),
