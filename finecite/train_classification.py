...
from finecite.model import (
    CustomTrainer,
    ExtractionModel,
    ClassificationModel,
    load_classifier,
    load_tokenizer_embedding_model,
    MODEL_DESCRIPTION,
)
...
def get_args():
    ...
    parser.add_argument(
        "--dataset", type=str, default="acl-arc", help="Dataset to use: 'acl-arc', 'act2', 'scicite', 'multicite'"
    )
    ...
    parser.add_argument("--learning_rate", type=float, default=2e-5, help="Learning rate for the base model")

    parser.add_argument(
        "--crf_learning_rate", type=float, default=0.005, help="Learning rate for the CRF layer (if used)"
    )

    parser.add_argument("--dropout", type=float, default=0.1, help="Dropout rate applied to the model")

    parser.add_argument("--save_model", action="store_true", help="If set, the model will be saved after training")

    parser.add_argument("--cached_data", action="store_true", help="Use cached preprocessed data if available")

    parser.add_argument("--debug", action="store_true", help="Run in debug mode (loads a small subset of data)")

    parser.add_argument("--debug_size", type=int, default=100, help="Number of examples to use in debug mode")

    parser.add_argument("--seed", type=int, default=4455, help="Random seed for reproducibility")
    ...
    # ext model dir
    args.ext_model_dir = re.sub(r'[.:*?"<>|\s-]', "_", (f"{OUT_DIR}/finecite/{args.ext_model}/{args.ext_type}"))
    set_seed(args.seed)
...
def main():
    ...
    if (
        args.cached_data
        and train_file in os.listdir(args.data_cache_dir)
        and test_file in os.listdir(args.data_cache_dir)
    ):
        ...
        train_ds = [
            {**example, "token_labels": torch.tensor(tok_lbl, dtype=torch.long)}
            for example, tok_lbl in zip(train_ds, train_lbls)
        ]
        test_ds = [
            {**example, "token_labels": torch.tensor(tok_lbl, dtype=torch.long)}
            for example, tok_lbl in zip(test_ds, test_lbls)
        ]
    ...
    num_pred_targets = len(
        [
            token
            for token in first_example["input_ids"]
            if token not in [tokenizer.cls_token_id, tokenizer.sep_token_id, tokenizer.pad_token_id]
        ]
    )
    print(
